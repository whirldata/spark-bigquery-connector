...
public class BigQueryBatchScan implements Scan, Batch, SupportsReportStatistics {
    ...
    public InputPartition[] planInputPartitions() {
        ...
        InputPartition[] bigQueryInputPartitions = new InputPartition[readSession.getStreamsList().size()];
        List<ReadStream> streamList = readSession.getStreamsList();

        for (int i = 0; i < streamList.size(); i++) {
            bigQueryInputPartitions[i] = new BigQueryInputPartition(bigQueryReadClientFactory, streamList.get(i).getName(),
                    readSessionCreatorConfig.toReadRowsHelperOptions(), createConverter(selectedFields, readSessionResponse, userProvidedSchema));
        }
